...
# uganda = [21,6,17,20,22,23,24]
# mali = [4,16,15,19,18,7,8,9,12]
...
def plotMessageRateOfMeters(meters = [4,6,7,8,9,12,15,16,17,20], dateStart=dt.datetime(2011,6,24), dateEnd=today):
    ...
    while currentDate < dateEnd:
        ...
        currentDate += dt.timedelta(days=1)
    print mrate
    tw.log.info('about to PLOT...')
    if len(meters)>5:
        fig, ax = plt.subplots(len(meters), 1, sharex = True, figsize=(11,8.5))
        for i,m in enumerate(meters):
            tw.log.info('meter = ' + str(meters[i]))
            thisAxes = ax[i]
